...
Of course, it is essential to specify parameters such as the exchange-correlation functional, convergence criteria, and so on. An example addressing this is shown below.
...
The parameters of the `defaults` dictionary are used in the `pymofscreen.default_calculators.calcs` function, which we suggest looking at before running PyMOFScreen for the first time. The job specifications and parameters can be freely changed using any of [ASE's parameters for VASP](https://wiki.fysik.dtu.dk/ase/ase/calculators/vasp.html).
...
1. `basepath`: The base directory where the output files will be handled. The converged results are stored in `/basepath/results`, and any errors are stored in `/basepath/errors`.
2. `mofpath`: The path where the starting MOF CIF files are located. Defaults to `/basepath/mofpath` if not specified.
3. `kpts_path` and `kppas`: If `kpts_path` is set to `Auto`, PyMOFScreen will automatically generate a k-point grid the `kppas` keyword argument. The `kppas` argument should be a list with two entries consisting of the low- and high-accuracy k-point density to use, in units of k-points per number of atoms (defaults to `kppas=[100,1000]`). Alternatively, you can instead provide a text file of the desired k-point grid for each CIF, specifying the path to this file via `kpts_path` and leaving `kppas=None` (see the `/examples` folder for an example k-points file).
4. `submit_script`: The name of the HPC Python job submission script.
5. `stdout_file`: The name of the standard output file created by the job scheduler.
